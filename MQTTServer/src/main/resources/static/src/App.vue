...
<template>
  <div id="app">
    <header>
      <h1>
        6-Axis Robotic Arm Twin
        <small>Kuka KR5-R850</small>
      </h1>
    </header>
    <div class="blueprint-container">
      <div class="overlay" v-if="loadingOverlay">
        <div class="overlay__inner">
          <div class="overlay__content">
            <span class="spinner"></span>
            <p class="spinner-text">{{ loaderMessage }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="mdl-grid">
        <div class="mdl-cell--2-col">
          ...
          <div
            v-for="job in jobs"
            @click="selectJob(job)"
            v-bind:class="{ job: true, 'selected-job': job === selectedJob }"
          >
            <h3>{{ job.name }}</h3>
          </div>
          <div class="job" style="text-align: center">
            <button @click="newJob()" class="new-job">
              <i class="material-icons" style="color: darkslategrey">add</i>
            </button>
            <div style="margin-top: 40px; color: darkslategray">
              ...
              <input
                ...
                @change="importJob($event)"
              />
              <button v-if="file" @click="uploadJob()">Import</button>
            </div>
          </div>
        </div>
        <div class="mdl-cell--10-col machine-container">
          <div class="overlay" v-if="loadingOverlay">
            <div class="overlay__inner">
              <div class="overlay__content">
                <span class="spinner"></span>
          ...
          <div class="job-controls">
            <div style="display: inline-block" v-if="selectedJob">
              <h2 v-if="!editTitle" @click="editTitle = true">
                {{ selectedJob.name }}
              </h2>
              <input
                style="margin: 18.4px 0; font-size: 1.5em"
                v-if="editTitle"
            ...
            <div style="display: inline-block; margin-left: 70px">
              <button class="control-button" @click="toggleAutoPlay()">
                <i class="material-icons" v-if="!autoPlay">play_arrow</i
                ><i class="material-icons" v-if="autoPlay">pause</i>
              </button>
              <button class="control-button" @click="toggleRecord()">
                <i
                  v-bind:style="{ color: recordingColor }"
              ...
              <button class="control-button" @click="exportJob()">
                <i class="material-icons">move_to_inbox</i>
              </button>
              <a
                @mouseleave="downloadLink = ''"
                v-if="downloadLink"
                :download="selectedJob.name + '.json'"
                :href="downloadLink"
...
export default {
  ...
  created() {
    ...
    // During initialization show loading overlay until "simulation_start" msg arrives
    self.showLoadingOverlay("Waiting for Blender simulation...");
    ...
    self.socket.addEventListener("message", function (event) {
      ...
      if (msg.topic === "state") {
        let jsonObj = JSON.parse(msg.message);
        let name = jsonObj.name;
        let machine = jsonObj.machine;

        self.currentState[machine] = name;
      }

      if (msg.topic === "context") {
        let jsonObj = JSON.parse(msg.message);
        let entity = Object.keys(jsonObj)[0];
        if (entity == "status") {
          switch (jsonObj["status"]) {
            case "simulation_start":
              self.currentState["conveyor"] =
                self.selectedJob.conveyorStates[0].name;
              self.currentState["roboticArm"] =
                self.selectedJob.roboticArmStates[0].name;
              self.currentState["slider"] =
                self.selectedJob.sliderStates[0].name;
              self.currentState["testingRig"] =
                self.selectedJob.testingRigStates[0].name;
              self.reset();
              break;
          }
        }
      ...
      if (msg.topic === "sensor") {
        ...
        if (data.entity === "RoboticArm") {
          ...
          self.context.roboticArmState.secondArmPosition =
            data.secondArmPosition;
          ...
          self.context.roboticArmState.headMountPosition =
            data.headMountPosition;
        ...
        if (data.entity === "TestingRig") {
          ...
          self.context.testingRigState.heatplateTemperature =
            data.heatplateTemperature;
        }
      }
    });
  },
  methods: {
    areMachinesInInitialState(currentStates) {
      return (
        ...
        this.selectedJob.roboticArmStates[0].name ==
          currentStates["roboticArm"] &&
        this.selectedJob.sliderStates[0].name == currentStates["slider"] &&
        this.selectedJob.testingRigStates[0].name == currentStates["testingRig"]
      );
    },
    newJob() {
      var job = {
    ...
    updateTitle() {
      ...
      xhr.open(
        "PUT",
        "http://localhost:8080/jobs/" + this.selectedJob.id,
        true
      );
      xhr.setRequestHeader("Content-type", "application/json; charset=utf-8");
    ...
    reset() {
      this.autoPlay = false;
      if (this.areMachinesInInitialState(this.currentState)) {
        this.hideLoadingOverlay();
      } else {
        this.showLoadingOverlay("Reset to initial states...");
      }
