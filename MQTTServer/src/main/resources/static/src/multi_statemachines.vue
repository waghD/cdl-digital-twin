...
<div class="state-machines">
    <div class="robotic-arm-state-machine">
        <span class="state-machine-header">Robotic Arm</span>
        ...
        <div class="machine">
            <div v-for="(s, index) in job.roboticArmStates" class="row">
                <div v-if="!s.choices" style="width: 260px; padding-top: 75px;">
                    ...
                    <div style="color: darkslategray; display: inline-block; vertical-align: top; margin-left: 20px;">
                        <i @click="toggleChoice(index)" v-if="!s.choices && index < job.roboticArmStates.length - 1 && !job.roboticArmStates[index + 1].choices" class="material-icons" style="font-size: 3em; position: relative; top: 45px;">
                            arrow_right_alt
                        </i>
                        <div style="display: inline-block; vertical-align: top;" v-if="index < job.roboticArmStates.length - 1 && job.roboticArmStates[index + 1].choices">
                        ...
                        <div>
                            <div v-for="(s1, i1) in s.choices.first" style="display: inline-block; vertical-align: top; width: 260px;">
                                <state-preview
                                        ...
                                        v-on:moveRight="moveRightChoice(s.choices.first,
                        $event)"
                                        v-on:remove="removeChoice(s, s.choices.first, $event, index)"
                                        v-on:recordPosition="saveChanges()"
                                >
                                </state-preview>
                                ...
                                <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i1 == s.choices.first.length - 1 && index == job.roboticArmStates.length - 1" @click="addStateChoice(s.choices.first)">
                        ...
                        <div>
                            <div v-for="(s2, i2) in s.choices.second" style="display: inline-block; vertical-align: top; width: 260px;">
                                <state-preview
                                        ...
                                        v-on:moveRight="moveRightChoice(s.choices.second,
                        $event)"
                                        v-on:remove="removeChoice(s, s.choices.second, $event, index)"
                                        v-on:recordPosition="saveChanges()"
                                >
                                </state-preview>
                                ...
                                <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i2 == s.choices.second.length - 1  && index == job.roboticArmStates.length - 1" @click="addStateChoice(s.choices.second)">
                                    <i class="material-icons">add</i>
                                </button>
                            </div>
                        </div>
     ...
     <div class="slider-state-machine">

            <span class="state-machine-header">Slider</span>
            <div v-if="showPopup" class="dim">
                <div class="popup">
                    <div v-if="selectedState && !showChoiceSettings">
                        <state-detail :state="selectedState" :socket="socket" :context="context" v-on:close="showPopup = false;" v-on:recordPosition="saveChanges()"></state-detail>
            ...
            <div class="machine">
                <div v-for="(s, index) in job.sliderStates" class="row">
                    <div v-if="!s.choices" style="width: 260px; padding-top: 75px;">
                        <state-preview
                                v-on:open="showState(s)"
                                :state="s"
                        ...
                        <div style="color: darkslategray; display: inline-block; vertical-align: top; margin-left: 20px;">
                            <i @click="toggleChoice(index)" v-if="!s.choices && index < job.sliderStates.length - 1 && !job.sliderStates[index + 1].choices" class="material-icons" style="font-size: 3em; position: relative; top: 45px;">
                                arrow_right_alt
                            </i>
                            <div style="display: inline-block; vertical-align: top;" v-if="index < job.sliderStates.length - 1 && job.sliderStates[index + 1].choices">
                                <div><i @click="choiceSettings(s, index)" class="material-icons" style="cursor: default; font-size: 2em; position: relative; top: 15px; left: 5px;">info</i></div>
                                <div><i @click="toggleChoice(index)" class="material-icons" style="cursor: default; transform: rotate(90deg); font-size: 3em; position: relative; top: 15px;">call_split</i></div>
                            </div>
                        </div>
                    ...
                    <div v-if="s.choices">
                        <div style="display: inline-block; vertical-align: top; margin-right: -40px;">
                            ...
                            $event)"
                                    ...
                                    <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i1 == s.choices.first.length - 1 && index == job.sliderStates.length - 1" @click="addStateChoice(s.choices.first)">
                                        <i class="material-icons">add</i>
                                    </button>
                                </div>
                            </div>
                            ...
                            $event)"
                                    ...
                                    <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i2 == s.choices.second.length - 1  && index == job.sliderStates.length - 1" @click="addStateChoice(s.choices.second)">
                                        <i class="material-icons">add</i>
                                    </button>
                                </div>
                            </div>
         ...
         <div class="conveyor-state-machine">
                <span class="state-machine-header">Conveyor</span>
                <div v-if="showPopup" class="dim">
                    <div class="popup">
                        <div v-if="selectedState && !showChoiceSettings">
                            <state-detail :state="selectedState" :socket="socket" :context="context" v-on:close="showPopup = false;" v-on:recordPosition="saveChanges()"></state-detail>
                ...
                <div class="machine">
                    <div v-for="(s, index) in job.conveyorStates" class="row">
                        <div v-if="!s.choices" style="width: 260px; padding-top: 75px;">
                            <state-preview
                                    v-on:open="showState(s)"
                                    :state="s"
                            ...
                            <div style="color: darkslategray; display: inline-block; vertical-align: top; margin-left: 20px;">
                                <i @click="toggleChoice(index)" v-if="!s.choices && index < job.conveyorStates.length - 1 && !job.conveyorStates[index + 1].choices" class="material-icons" style="font-size: 3em; position: relative; top: 45px;">
                                    arrow_right_alt
                                </i>
                                <div style="display: inline-block; vertical-align: top;" v-if="index < job.conveyorStates.length - 1 && job.conveyorStates[index + 1].choices">
                                    <div><i @click="choiceSettings(s, index)" class="material-icons" style="cursor: default; font-size: 2em; position: relative; top: 15px; left: 5px;">info</i></div>
                                    <div><i @click="toggleChoice(index)" class="material-icons" style="cursor: default; transform: rotate(90deg); font-size: 3em; position: relative; top: 15px;">call_split</i></div>
                                </div>
                            </div>
                        ...
                        <div v-if="s.choices">
                            <div style="display: inline-block; vertical-align: top; margin-right: -40px;">
                                ...
                                $event)"
                                        ...
                                        <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i1 == s.choices.first.length - 1 && index == job.conveyorStates.length - 1" @click="addStateChoice(s.choices.first)">
                                            <i class="material-icons">add</i>
                                        </button>
                                    </div>
                                </div>
                                ...
                                $event)"
                                        ...
                                        <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i2 == s.choices.second.length - 1  && index == job.conveyorStates.length - 1" @click="addStateChoice(s.choices.second)">
                                            <i class="material-icons">add</i>
                                        </button>
                                    </div>
                                </div>
              ...
              <div class="testrig-state-machine">
                            <span class="state-machine-header">Test Rig</span>
                            <div v-if="showPopup" class="dim">
                                <div class="popup">
                                    <div v-if="selectedState && !showChoiceSettings">
                                        <state-detail :state="selectedState" :socket="socket" :context="context" v-on:close="showPopup = false;" v-on:recordPosition="saveChanges()"></state-detail>
                            ...
                            <div class="machine">
                                <div v-for="(s, index) in job.testingRigStates" class="row">
                                    <div v-if="!s.choices" style="width: 260px; padding-top: 75px;">
                                        <state-preview
                                                v-on:open="showState(s)"
                                                :state="s"
                                        ...
                                        <div style="color: darkslategray; display: inline-block; vertical-align: top; margin-left: 20px;">
                                            <i @click="toggleChoice(index)" v-if="!s.choices && index < job.testingRigStates.length - 1 && !job.testingRigStates[index + 1].choices" class="material-icons" style="font-size: 3em; position: relative; top: 45px;">
                                                arrow_right_alt
                                            </i>
                                            <div style="display: inline-block; vertical-align: top;" v-if="index < job.testingRigStates.length - 1 && job.testingRigStates[index + 1].choices">
                                                <div><i @click="choiceSettings(s, index)" class="material-icons" style="cursor: default; font-size: 2em; position: relative; top: 15px; left: 5px;">info</i></div>
                                                <div><i @click="toggleChoice(index)" class="material-icons" style="cursor: default; transform: rotate(90deg); font-size: 3em; position: relative; top: 15px;">call_split</i></div>
                                            </div>
                                        </div>
                                    ...
                                    <div v-if="s.choices">
                                        <div style="display: inline-block; vertical-align: top; margin-right: -40px;">
                                            ...
                                            $event)"
                                                    ...
                                                    <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i1 == s.choices.first.length - 1 && index == job.testingRigStates.length - 1" @click="addStateChoice(s.choices.first)">
                                                        <i class="material-icons">add</i>
                                                    </button>
                                                </div>
                                            </div>
                                            ...
                                            $event)"
                                                    ...
                                                    <button class="new" style="position: relative; top: 45px; left: 20px;" v-if="i2 == s.choices.second.length - 1  && index == job.testingRigStates.length - 1" @click="addStateChoice(s.choices.second)">
                                                        <i class="material-icons">add</i>
                                                    </button>
                                                </div>
                                            </div>
